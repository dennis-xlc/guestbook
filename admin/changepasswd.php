...
<?php
  ...
  function redirect2UserInfo() {
    echo '<script type="text/javascript">alert("恭喜您，密码修改成功！");window.location.href = "./listnonreply.php";</script>';
  }

  function updatePasswd($adminId, $passwd, $newpasswd) {
    include("../config/config.php");
    $conn = mysql_connect($dbhost, $dbuser, $dbpassword, $database) or die (mysql_error());
    mysql_select_db($database, $conn) or die (mysql_error());

    $qry = "select * from admin where id = '".$adminId."' and password = '".$passwd."'";
    $result = mysql_query($qry, $conn);
    $rowNum = mysql_num_rows($result);
    if($rowNum <= 0) {
      phpAlert("对不起，密码不正确！");
    } else {
      $qry = "update admin set password = '".$newpasswd."' where id = '".$adminId."'";
      $result = mysql_query($qry, $conn);
      if($result) {
        redirect2UserInfo();
      }
  ...
  if(isset($_POST['submit'])) {
    ...
    updatePasswd($adminId, $passwd, $newpasswd);
  }
?>
<body>
